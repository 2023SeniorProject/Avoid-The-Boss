...
namespace DX
{
    ...
    public:
        ...
        UINT AllocateDescriptorIndices(UINT numDescriptors, UINT firstDescriptorIndexToUse = UINT_MAX)
        {
            ...
            firstDescriptorIndexToUse = AllocateDescriptor(&cpuhandle, firstDescriptorIndexToUse);

            for (UINT i = 1; i < numDescriptors; i++)
            {
                D3D12_CPU_DESCRIPTOR_HANDLE cpuhandle2 = D3D12_CPU_DESCRIPTOR_HANDLE();

                AllocateDescriptor(&cpuhandle2, firstDescriptorIndexToUse + i);
            }
...
inline float inlerp(float a, float b, float t)
{
    return a + t * (b - a);
}
...
inline float CalculateAnimationInterpolant(float elapsedTime, float cycleDuration)
{
	...
	return inlerp(0.0f, 1.0f, curLinearCycleTime);
}
    ...
    D3D12_SRV_DIMENSION srvDimension = D3D12_SRV_DIMENSION_TEXTURE2D)
{
    ...
    // l-value 오류 수정
// Corrected l-value initialization for CD3DX12_HEAP_PROPERTIES
    CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_UPLOAD);
    CD3DX12_HEAP_PROPERTIES* pHeapProperties = &heapProperties;

    // Corrected l-value initialization for CD3DX12_RESOURCE_DESC
    CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(uploadBufferSize);
    CD3DX12_RESOURCE_DESC* pDesc = &resourceDesc;
    ...
    CD3DX12_RESOURCE_BARRIER func = CD3DX12_RESOURCE_BARRIER::Transition(*ppResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    commandList->ResourceBarrier(1, &func);
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE* gpuHandle)
{
    ...
    // l-value 오류 수정
    CD3DX12_HEAP_PROPERTIES pHeapProperties2 = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
    CD3DX12_RESOURCE_DESC pDesc2 = CD3DX12_RESOURCE_DESC::Buffer(uploadBufferSize);
    // Create the GPU upload buffer.
    ThrowIfFailed(
        device->CreateCommittedResource(
            &pHeapProperties2,
    ...
    // l-value 오류 수정
    CD3DX12_RESOURCE_BARRIER fuc = CD3DX12_RESOURCE_BARRIER::Transition(*ppResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    commandList->ResourceBarrier(1, &fuc);
    ...
    const wchar_t* resourceName = nullptr)
{
    ...
    CD3DX12_RESOURCE_DESC resource = CD3DX12_RESOURCE_DESC::Buffer(vertexDataSize);
    ThrowIfFailed(device->CreateCommittedResource(
        &heap,
        D3D12_HEAP_FLAG_NONE,
        &resource,
...
)
{
    ...
    CD3DX12_RESOURCE_DESC resource = CD3DX12_RESOURCE_DESC::Buffer(indexDataSize);
    ...
    PIXBeginEvent(commandList, 0, L"Copy buffer data to default resource...");
    {
        ...
        CD3DX12_RESOURCE_BARRIER func = CD3DX12_RESOURCE_BARRIER::Transition(destBuffer, D3D12_RESOURCE_STATE_COPY_DEST, outResourceState);
        commandList->ResourceBarrier(1, &func);
    }
