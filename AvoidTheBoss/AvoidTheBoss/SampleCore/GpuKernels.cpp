        ...
        UINT invocationIndex)   // per frame invocation index)
	{
		...
		SIZE_T readBackBaseOffset = frameIndex * m_resultSize;
		{
			...
			for (UINT i = 0; i < m_csReduceSumOutputs.size(); i++)
			{
				...
				{
                    CD3DX12_RESOURCE_BARRIER func = CD3DX12_RESOURCE_BARRIER::Transition(m_csReduceSumOutputs[i].resource.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
				...
				{
                    CD3DX12_RESOURCE_BARRIER func = CD3DX12_RESOURCE_BARRIER::Transition(m_csReduceSumOutputs.back().resource.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
					commandList->ResourceBarrier(1, &func);
				}
			}
		...
		m_readbackResources[invocationIndex]->Unmap(0, &range);
	}
