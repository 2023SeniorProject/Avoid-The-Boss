...
//LoadPBRTScene : 씬에 들어갈 메쉬들을 묶어 .Pbrt(Physically Based Rendering Toolkit) 형식 파일로 임포팅한다.
// 
//LoadPBRTScene() 함수의 BLAS 입력 구조들을 binParser.Parse()로 바꾼다.
//LoadBinScene()함수를 LoadScnen()함수에 추가한다.  또는 이 함수에 추가한다. 
//LoadBinScene()에서 RasterDataStructureAlignmentToDXR() 호출하여 정렬된 데이터 구조로 바꾼다.
