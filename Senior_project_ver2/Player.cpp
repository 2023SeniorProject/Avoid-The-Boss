	...
	CPlayer(nMeshes)
{
	...
	20.0f, 4.0f, XMFLOAT4(5.0f, 0.0f, 0.0f, 0.0f));
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case FIRST_PERSON_CAMERA:
		...
		m_pCamera->SetTimeLag(0.25f); //--- 카메라 지연 설정
...
CLandPlayer::CLandPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) : CPlayer(nMeshes)
{
	m_pCamera = ChangeCamera(THIRD_PERSON_CAMERA, 0.0f);

	CMapLand* pTerrain = (CMapLand*)pContext;
	...
	SetPosition(XMFLOAT3(pTerrain->GetWidth() * 0.5f, fHeight+1500,
		pTerrain->GetLength() * 0.5f));
...
CLandPlayer::~CLandPlayer()
{
}

CCamera* CLandPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
...
void CLandPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	XMFLOAT3 xmf3PlayerPosition = GetPosition();
	CMapLand* pTerrain = (CMapLand*)m_pPlayerUpdatedContext;
...
void CLandPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
	...
	CMapLand* pLand = (CMapLand*)m_pCameraUpdatedContext;
	float fHeight = pLand->GetHeight(xmf3CameraPosition.x, xmf3CameraPosition.z) +
		5.0f;
