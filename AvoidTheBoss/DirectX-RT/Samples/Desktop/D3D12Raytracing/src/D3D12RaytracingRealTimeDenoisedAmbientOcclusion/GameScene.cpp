...
void GameScene::CreateAuxilaryDeviceResources()
{
    auto device = m_deviceResources->GetD3DDevice();
...
void GameScene::GetGrassParameters(GenerateGrassStrawsConstantBuffer_AppParams* params, UINT LOD, float totalTime)
{
...
void GameScene::LoadSceneGeometry()
{
    LoadPBRTScene();
#if RENDER_GRASS_GEOMETRY
...
void GameScene::InitializeGrassGeometry()
{
    auto device = m_deviceResources->GetD3DDevice();
    auto commandList = m_deviceResources->GetCommandList();
    auto commandQueue = m_deviceResources->GetCommandQueue();
...
void GameScene::InitializeAllBottomLevelAccelerationStructures()
{
    auto device = m_deviceResources->GetD3DDevice();
