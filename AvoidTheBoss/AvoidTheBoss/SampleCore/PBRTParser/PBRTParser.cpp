...
namespace PBRTParser
{
    ...
    {
        SceneParser::Material material;
		material.m_Opacity = SceneParser::Vector3(1, 1, 1);
        string materialType;
        ...
        auto pfnParseMaterialColor = [&](istream &inStream, SceneParser::Vector3 &color, string &textureFileName)
        {
...
#
    ...
    string PBRTParser::GenerateCheckerboardTexture(string fileName, float uScaleFloat, float vScaleFloat, SceneParser::Vector3 color1, SceneParser::Vector3 color2)
    {
        ...
        vector<SceneParser::Vector3> imageData;
    ...
    void PBRTParser::GenerateBMPFile(string fileName, _In_reads_(width * height)SceneParser::Vector3 *pImageData, UINT width, UINT height)
    {
    ...
    {
        SceneParser::Mesh *pMesh;
        if (GetCurrentAttributes().GetType() == Attributes::AreaLight)
        {
            outputScene.m_AreaLights.push_back(
                SceneParser::AreaLight(GetCurrentAttributes().GetAreaLightAttribute().m_lightColor));
            pMesh = &outputScene.m_AreaLights.back().m_Mesh;
        }
        else
        {
            outputScene.m_Meshes.push_back(SceneParser::Mesh());
    ...
    void PBRTParser::InitializeCameraDefaults(SceneParser::Camera& camera)
    {
        camera.m_LookAt = SceneParser::Vector3(0.0f, 0.0f, 1.0f);
        camera.m_Position = SceneParser::Vector3(0.0f, 0.0f, 0.0f);
        camera.m_Up = SceneParser::Vector3(0.0f, 1.0f, 0.0f);
